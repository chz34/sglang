...
from mindformers.experimental.infer.core import get_act_func, get_norm
...
from .ms_attention import MsAttnBackend
...
class ParallelAttention(nn.Cell):
    ...
    def __init__(self, config, layer_number, attention_type="self_attn", attn_mask_type=None):
        ...
        if self.use_gqa:
            ...
            if self.kv_num_heads < self.tp_group_size:
                self.kv_num_heads = self.tp_group_size
        ...
        if self.attn_type == "self_attn":
            ...
            raise NotImplementedError(
                f"attention_type(str) should be 'self_attn', but got {self.attn_type}")
        self.reshape = ops.Reshape()
        self.cast = ops.Cast()
        ...
        self.attn = MsAttnBackend(self.config,
    ...
    def construct(self, x, batch_valid_length, freqs_cis=None,
        ...
        if self.qkv_concat:
            ...
            query = self.reshape(query, (num_t, self.hidden_size_per_partition))
            key = self.reshape(key, (num_t, self.kv_hidden_size_per_partition))
            value = self.reshape(value, (num_t, self.kv_hidden_size_per_partition))
        else:
        ...
        key_out = self.attn(key, value, key_cache=key_cache, value_cache=value_cache, out_cache_loc=out_cache_loc)
        query = ops.depend(query, key_out)

        if self.is_first_iteration:
            context_layer = self.attn.extend(query, key, value, attn_mask, alibi_mask, None, None,
                                             q_seq_lens, batch_valid_length)
        else:
            context_layer = self.attn.decode(query, batch_valid_length, attn_mask, q_seq_lens, key_cache, value_cache,
        ...
        if self.kv_num_heads >= self.tp_group_size:
            if self.kv_num_heads % self.tp_group_size != 0:
                raise ValueError(
                    ...
                    f"but got kv_num_heads {self.kv_num_heads} and tp_group_size {self.tp_group_size}"
                )
        else:
            # Number of KV heads is less than TP size, will replicate the KV heads.
            if self.tp_group_size % self.kv_num_heads != 0:
                raise ValueError(
...
class ParallelTransformerLayer(nn.Cell):
    ...
    def construct(self, x, freqs_cis=None, mask=None, batch_valid_length=None,
                  prefix_keys_values=None, q_seq_lens=None, key_cache=None, value_cache=None, out_cache_loc=None,
                  token_cache_loc=None, kv_mask=None):
        ...
        # attention.
        attention_output = self.attention(norm_output, batch_valid_length, freqs_cis,
                                          mask, prefix_keys_values=prefix_keys_values,
                                          q_seq_lens=q_seq_lens, key_cache=key_cache, value_cache=value_cache,
                                          out_cache_loc=out_cache_loc, token_cache_loc=token_cache_loc,
                                          kv_mask=kv_mask)
        # residual-connection.
...
class ParallelTransformer(nn.Cell):
    ...
    def construct(self, tokens: Tensor, batch_valid_length=None, batch_index=None, zactivate_len=None,
                  prefix_keys_values=None, position_ids=None, attention_mask=None,
                  q_seq_lens=None, key_cache=None, value_cache=None, out_cache_loc=None,
                  token_cache_loc=None, kv_mask=None):
        """
        ...
        for i in range(self.num_layers):
            ...
            hidden_states = self.layers[i](hidden_states, freqs_cis, mask, batch_valid_length=batch_valid_length,
                                           ...
                                           token_cache_loc=token_cache_loc, kv_mask=kv_mask)
